...
/***************************************************************************/	
	...
	function penjadwalan(){
		...
		if(!empty($_POST)){
			$this->form_validation->set_rules('semester_tipe','Semester','xss_clean|required');
			$this->form_validation->set_rules('tahun_akademik','Tahun Akademik','xss_clean|required');
			$this->form_validation->set_rules('pop_unit','Jumlah Populiasi','xss_clean|required');
			$this->form_validation->set_rules('counter','Probabilitas CrossOver','xss_clean|required');
			$this->form_validation->set_rules('limiter','Probabilitas Mutasi','xss_clean|required');
			$this->form_validation->set_rules('locker','Jumlah Generasi','xss_clean|required');
			...
			{				
				...
				$tahun_akademik = $this->input->post('tahun_akademik');
				$pop_unit = $this->input->post('pop_unit');
				...
				if($rs_data->num_rows() == 0){
					...
					for($i = 0;$i < $locker;$i++ ){
						...
						$rule->Seleksi($fitness);
						$rule->StartCrossOver();
						
						$fitnessAfterMutation = $rule->Mutasi();
						...
						for ($j = 0; $j < count($fitnessAfterMutation); $j++){
							...
							if($fitnessAfterMutation[$j] == 1){
								...
								$jadwal_kuliah = $rule->GetIndividu($j);
